...
for(epoch in 1:epochs){
  ...
  for(index in 1:num_batches){
    ...
    noise <- runif(n = batch_size*latent_size, min = 0, max = 1) %>%
    ...
    # discriminator
    noise <- runif(2*batch_size*latent_size, min = 0, max = 1) %>%
  ...
  # Generate a new batch of noise
  noise <- runif(num_test*latent_size, min = 0, max = 1) %>%
  ...
  noise <- runif(2*latent_size, min = 0, max = 1) %>%
    matrix(nrow = 2, ncol = latent_size)
