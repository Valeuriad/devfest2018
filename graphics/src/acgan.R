...
plotImages <- function(image){
  ...
  grid.raster((EBImage::rotate(image, 180)+1)/2)
}
...
build_generator <- function(latent_size){
  ...
  cls <-  image_class %>%
    layer_embedding(
      input_dim = 2, output_dim = latent_size, 
...
build_discriminator <- function(){
  ...
  aux <- features %>%
    layer_dense(1, activation = "sigmoid", name = "auxiliary")
...
# range [-1, 1]
d <- readRDS("../data/proc/gm/data.dat")
dd <- readRDS("../data/proc/pg/data.dat")
d <- abind(d,dd,along = 1)
labels = array(0, dim(d)[1])
labels[1:dim(d)[1]] = 1
d = abind(d,d, along = 1)
...
dataset$train$y <- labels[train_indexes]
dataset$test$x <- d[test_indexes,,]
dataset$test$y <- labels[test_indexes]

dataset$train$x <- array_reshape(dataset$train$x, c(length(train_indexes), 1, 28, 28))
dataset$test$x <- array_reshape(dataset$test$x, c(length(test_indexes), 1, 28, 28))
...
for(epoch in 1:epochs){
  ...
  for(index in 1:num_batches){
    ...
    label_batch <- dataset$train$y[batch]
    
    # Sample some labels from p_c
    sampled_labels <- sample(0:1, batch_size, replace = TRUE) %>%
      matrix(ncol = 1)
    ...
    sampled_labels <- sample(0:1, size = 2*batch_size, replace = TRUE) %>%
      matrix(ncol = 1)
  ...
  sampled_labels <- sample(0:1, size = num_test, replace = TRUE) %>%
    matrix(ncol = 1)
  generated_images <- predict(generator, list(noise, sampled_labels))
  
  X <- abind(dataset$test$x, generated_images, along = 1)
  y <- c(rep(1, num_test), rep(0, num_test)) %>% matrix(ncol = 1)
  aux_y <- c(dataset$test$y, sampled_labels) %>% matrix(ncol = 1)
  ...
  sampled_labels <- sample(0:1, size = 2*num_test, replace = TRUE) %>%
    matrix(ncol = 1)
  ...
  img[img >= 1] = 1-.Machine$double.eps
  img[img <= -1] = -1+.Machine$double.eps
  ((img+1 )/2) %>% as.raster() %>%
    plot()
